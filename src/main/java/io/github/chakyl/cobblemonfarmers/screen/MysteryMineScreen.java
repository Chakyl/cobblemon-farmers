...
public class MysteryMineScreen extends AbstractContainerScreen<MysteryMineMenu> {
    public static final ResourceLocation TEXTURE = new ResourceLocation(CobblemonFarmers.MODID, "textures/gui/mystery_mine.png");
    ...
    private static final Component CONTAINER_LABEL = Component.translatable("gui.cobblemon_farmers.mystery_mine");
    ...
    protected void renderLabels(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY) {
        ...
        if (pokemon != null) {
            ...
            GuiUtilsKt.drawText(pGuiGraphics, COBBLE_FONT, Component.translatable("info.cobblemon_farmers.mystery_mine.type." + pokemon.getPrimaryType().getName().toLowerCase()), centralX + 90, 44, false, 0xFFFFFFFF, false, 0, 0);
            if (pokemon.getSecondaryType() != null) {
                GuiUtilsKt.drawText(pGuiGraphics, COBBLE_FONT, Component.translatable("info.cobblemon_farmers.mystery_mine.type." + pokemon.getSecondaryType().getName().toLowerCase()), centralX + 90, 58, false, 0xFFFFFFFF, false, 0, 0);
            }
        }
    }
    ...
    private List<Component> getStatImpactTooltip() {
        MysteryMineRecipe recipe = this.menu.getCurrentRecipe();
        if (recipe != null) {
            List<Component> tooltipComponents = new ArrayList<>(List.of(
                    Component.translatable("jei.cobblemon_farmers.mystery_mine.speed_stat", recipe.getSpeedStat().getDisplayName()).withStyle(ChatFormatting.AQUA)
            ));
            if (recipe.getMultStat() != null) {
                tooltipComponents.add(Component.translatable("jei.cobblemon_farmers.mystery_mine.mult_stat", recipe.getMultStat().getDisplayName()).withStyle(ChatFormatting.GREEN));
            }
    ...
    private Component getCraftingTimeTooltip() {
        ...
        if (remainingSeconds > 0 && this.menu.getScaledProgress() > 0) {
            ...
            return Component.translatable("tooltip.cobblemon_farmers.mystery_mine.processing_time", formattedTime);
        } else {
            return Component.translatable("tooltip.cobblemon_farmers.mystery_mine.processing_time", "0:00 Seconds");
        }
    }
