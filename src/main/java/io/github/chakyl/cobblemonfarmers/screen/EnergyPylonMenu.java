...
public class EnergyPylonMenu extends AbstractWorkerMenu {
    public final EnergyPylonBlockEntity blockEntity;
    ...
    public EnergyPylonMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(5));
    }

    public EnergyPylonMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(CobblemonFarmersRegistery.MenuRegistry.ENERGY_PYLON.get(), pContainerId, inv, entity, data);
        checkContainerSize(inv, 2);
        blockEntity = ((EnergyPylonBlockEntity) entity);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, Direction.DOWN).ifPresent(iItemHandler -> {
            this.addSlot(new SlotItemHandler(iItemHandler, 0, 143, 20));
        });
    ...
    protected void getPartySlots() {
        for (int i = 0; i < 6; ++i) {
            this.partySlots.add(this.addSlot(new EnergyPylonMenu.PartySlot(this.partyContainer, i, i % 2 == 0 ? 186 : 217, (((i / 2) * 31) + (i % 2 == 0 ? 20 : 28)))));
    ...
    private class PartySlot extends WorkstationPartySlot {
        ...
        public Optional<ItemStack> tryRemove(int pCount, int pDecrement, Player pPlayer) {
            EnergyPylonMenu.this.transferFromPartyToWorkerSlot(player, this);
            return Optional.empty();
        }
    }

    private void transferFromPartyToWorkerSlot(Player player, EnergyPylonMenu.PartySlot partySlot) {
