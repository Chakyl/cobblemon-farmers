...
public class CrystalBallMenu extends AbstractWorkerMenu {
    public final CrystalBallBlockEntity blockEntity;
    ...
    public CrystalBallMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(5));
    }

    public CrystalBallMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(CobblemonFarmersRegistery.MenuRegistry.CRYSTAL_BALL.get(), pContainerId, inv, entity, data);
        checkContainerSize(inv, 2);
        blockEntity = ((CrystalBallBlockEntity) entity);
    ...
    protected void getPartySlots() {
        for (int i = 0; i < 6; ++i) {
            this.partySlots.add(this.addSlot(new CrystalBallMenu.PartySlot(this.partyContainer, i, i % 2 == 0 ? 186 : 217, (((i / 2) * 31) + (i % 2 == 0 ? 20 : 28)))));
    ...
    private class PartySlot extends WorkstationPartySlot {
        ...
        public Optional<ItemStack> tryRemove(int pCount, int pDecrement, Player pPlayer) {
            CrystalBallMenu.this.transferFromPartyToWorkerSlot(player, this);
            return Optional.empty();
        }
    }

    private void transferFromPartyToWorkerSlot(Player player, CrystalBallMenu.PartySlot partySlot) {
